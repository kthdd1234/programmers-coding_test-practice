...
function solution(n, arr1, arr2) {
    let emptyList = Array.from({ length: n });
    var answer = emptyList.map((_, idx) => {
        let acc = '';
        let list = [arr1[idx], arr2[idx]].map((number) => {
                let binary = number.toString(2);
                return binary.padStart(n, '0');
        });
        
        emptyList.forEach((_, idx) => {
            let [num1, num2] = [parseInt(list[0][idx]), parseInt(list[1][idx])];
            num1 || num2 ? acc += '#' : acc  += ' ';
        });
